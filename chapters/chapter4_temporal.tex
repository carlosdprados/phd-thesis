...
\paragraph{Heterogeneity as a design resource.} The three tiers give a consistent --- and honest --- account of what timescale heterogeneity is worth. Where the task demands memory at many lags at once, namely random-input memory capacity (a $1.49\times$ broadening; \cref{fig:ch4_mc_curve}) and real-physiology context reconstruction ($+0.012$ in held-out $R^2$ over the best homogeneous bank; \cref{tab:ch4_physio}), heterogeneity is a measurable computational resource. Where a single slow timescale already dominates the discriminative information, as in the sustained \WESAD{} affect labels, it is a generality and robustness hedge rather than a source of additional accuracy. Crucially for the materials argument of this thesis, that heterogeneity is \emph{cheap and intrinsic}: it comes for free with a solution-processed device family, through composition spread, device-to-device scatter, and --- as further, illustrative knobs catalogued in \cref{ch:comparative} but not required by the quantitative banks here --- electrolyte chemistry and drive amplitude.
