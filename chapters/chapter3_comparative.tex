...
\begin{figure}[t]
  \centering
  \figplaceholder{6.2cm}{Composition heatmaps over the PEO ($0.3,0.6,1.2$) $\times$ salt ($0.045,0.09,0.18$) grid: (a) on--off ratio, (b) normalised loop area, (c) fading-memory $t_{1/2}$. Per-cell median, $n=2$--$4$ devices.}
...
First, the steady-state switching window contracts as the \PEO{} fraction rises. At fixed salt mass fraction \num{0.09}, the mean on--off ratio falls from \(\approx 3.7\) at \PEO{}~\num{0.3} to \(\approx 2.5\) at \PEO{}~\num{1.2}, and the normalised loop area falls correspondingly from \(\approx 0.31\) to \(\approx 0.18\); the activation voltage is, by contrast, essentially flat across the whole grid (\SIrange{2.2}{2.4}{\volt}), indicating that composition tunes how \emph{much} the conductance moves rather than the voltage at which it begins to move. The largest window in the grid occurs at the low-salt, low-\PEO{} corner (\PEO{}~\num{0.3}, salt~\num{0.045}: on--off ratio \(\approx 7.8\), area \(\approx 0.5\)).

Second, potentiation weakens as the \PEO{} fraction rises. The growth of the conductance ratio with pulse number (measured as the slope on log--log axes) decreases monotonically with \PEO{} content, from \(\approx 1.1\) at \PEO{}~\num{0.3} to \(\approx 0.4\) at \PEO{}~\num{1.2}, and the peak conductance ratio falls over more than two orders of magnitude across the grid (from several hundred-fold at the strongest cells to a few-fold at the weakest). In roughly half of the cells the potentiation \emph{turns over} --- the conductance ratio peaks and then declines by the largest pulse counts ($N\sim\numrange{100}{300}$) --- which sets a practical ceiling on the usable pulse count and is itself a design constraint for \cref{ch:temporal}.

Third, and most important for the rest of the thesis, the fading-memory timescale is composition-tunable over roughly \SIrange{2}{20}{\second}. The longest retention occurs at \PEO{}~\num{0.3} (with \(t_{1/2}\approx\SI{19}{\second}\) at salt~\num{0.09}, and an identified \(\tau\approx\SI{20}{\second}\)); increasing the \PEO{} fraction to \num{0.6} or \num{1.2} shortens the timescale to a few seconds. The same ordering is reproduced by the instrument-side single-exponential time constant computed independently, which provides an internal cross-check on the trend. The device-to-device scatter within each cell is substantial and is not treated here as mere noise: the spread of timescales \emph{across} the grid, together with the within-cell variability, constitutes the heterogeneous bank of fading-memory elements that \cref{ch:temporal} exploits.

Taken together, these three observations support a single physical reading: increasing the proportion of the ion-transport phase produces a weaker, faster-relaxing memristive response. This is the quantitative spine of the chapter, and the one result that rests on replicated devices.
...
\begin{figure}[t]
  \centering
  \figplaceholder{5.4cm}{Fading-memory time constant by chemistry, with $n$ labelled on each bar: host (PEO vs TMPE, Li-triflate); anion (triflate vs TFSI, in PEO); cation (Li/Na/K, in TMPE/TFSI).}
...
\begin{figure}[t]
  \centering
  \figplaceholder{5.2cm}{Depotentiation decay of a single device measured under two drive amplitudes (3\,V and 6\,V write); the higher amplitude yields a $\sim3\times$ longer apparent relaxation time.}
