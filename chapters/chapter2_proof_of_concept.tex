...
%% --- Figure placeholder ---------------------------------------------------
%% Draws a sized, framed box in place of a not-yet-generated figure so that
%% the float, its caption, and its \label exist and all \cref{fig:...} resolve
%% while the chapter still compiles cleanly (no missing-image errors).
%% To finalise a figure, replace \figplaceholder{<height>}{<desc>} with the
%% real \includegraphics{...}. Source captions/data: handouts/03_chapter2_figures_needed.md.
\newcommand{\figplaceholder}[2]{%
  ...
  \fbox{\parbox[c][#1][c]{0.85\linewidth}{%
    ...
    \texttt{handouts/03\_chapter2\_figures\_needed.md}%
  }}%
}
...
The literature already contained, at the start of this work, several partial demonstrations of these functions in different architectures, as reviewed in \cref{subsec:organic_history}. The polyaniline-based electrochemical junctions of Erokhin and co-workers~\cite{Erokhin2005, Smerieri2008, Berzina2009} demonstrated ion-mediated analogue switching, but in a multi-layer geometry rather than a single composite film. The \(\ce{[Ru(bpy)3]^{2+}}\) light-emitting memristor of Zakhidov and co-workers~\cite{Zakhidov2010} demonstrated bistable ionic redistribution, but without analogue gradation or full reversibility. The redox-active two-terminal device of Liu and co-workers~\cite{Liu2016} demonstrated tunable short-term to long-term memory transitions, but relied on partially irreversible redox transformations of the kind documented for redox-based organic memory more broadly~\cite{Heremans2011}. Organic electrochemical transistors achieved reversible analogue control under biologically realistic learning rules~\cite{Gkoupidenis2015, Xu2016, VanDeBurgt2017, Fuller2019}, but at the architectural cost of an additional gate terminal and a contacted electrolyte volume. Each of these implementations covered part of the specification developed in \cref{subsec:why_hw} (analogue, reversible, multi-state, low-energy conductance changes set by electrical pulses), but the combination of those features in a single two-terminal organic device with an interpretable ion-mediated mechanism remained, at that point, an open problem.
...
The chapter shows that the composite supports, in a single 2-T platform, a full set of synaptic functions that fall into two families. The first family comprises three dynamical measurements: current--voltage hysteresis characteristic of memristive switching, potentiation as a function of the number of applied voltage pulses, and depotentiation as a function of the delay before read-out. These three measurements recur, on the same 2-T device geometry, throughout the comparative work of Chapters~3 and~4; they are introduced first, and as an explicitly named set, so that the reader meets them here once rather than encountering them repeatedly in the later chapters without context. The second family comprises extended synaptic protocols that build on the first and are specific to this chapter: excitatory post-synaptic current (\EPSC{}) responses, demonstrated here with four readily distinguishable readout states; the resolution of the depotentiation measurement into a short-lived and a longer-lived memory regime, denoted \STM{} (STM) and \LTM{} (LTM) following the convention of the artificial-synapse literature; and spike-timing dependent plasticity (\STDP{}) with a time constant in the biologically realistic range. The interpretation tying these observations together is developed in \cref{sec:mechanism}: the fast component is associated with displacement of the weakly bound triflate anion, while the slow component is associated with displacement of the lithium cation from its hard oxygen coordination environment. The same interpretation yields the cation-substitution prediction tested in Chapter~3.

Within the arc of the thesis, the present chapter is narrow in role. It is the only chapter in which the full synaptic suite (EPSC, the separated \STM{}/\LTM{} reading of the retention data, and STDP) is treated as the main evidence. The later chapters retain only the three dynamical measurements just described --- I--V hysteresis, variable-number-of-pulses potentiation, and variable-delay depotentiation --- shared across the whole comparative corpus. The two specific handoffs onto the rest of the thesis are taken up at the end of the chapter.

This chapter is organised as follows. \Cref{sec:materials_design} sets out the molecular composition and the design rationale of the composite. \Cref{sec:device_fab} describes the fabrication protocol and the morphological characterisation of the active layer. \Cref{sec:elec_char} reports the full electrical characterisation suite. \Cref{sec:mechanism} develops the mechanistic interpretation of the two memory regimes in terms of cation--host coordination chemistry, together with the injection-limited and bulk-doping descriptions of the polymer film. \Cref{sec:stability} addresses device stability and reproducibility. \Cref{sec:comparison} contextualises the results against the state of the art at the time of publication. \Cref{sec:ch2_summary} closes the chapter and sets up the comparative and computational work that follows.
...
The semiconducting component of the composite is \SY{}, the trivial name for the high-molecular-weight PPV copolymer poly[\{2,5-di(3$'$,7$'$-dimethyloctyloxy)-1,4-phenylenevinylene\}-\textit{co}-\{3-(4$'$-(3$''$,7$''$-dimethyloctyloxy)phenyl)-1,4-phenylenevinylene\}-\textit{co}-\{3-(3$'$-(3$'$,7$'$-dimethyloctyloxy)phenyl)-1,4-phenylenevinylene\}] (commercial name: PDY-132, Merck). The chemical structure features a $\pi$-conjugated backbone of alternating phenylene and vinylene units with branched alkoxy side chains that confer solubility in common organic solvents and reduce intermolecular packing to mitigate crystallisation-induced phase separation in blends.

The frontier orbital levels of \SY{} (\(E_{\mathrm{HOMO}} \approx \SI{-5.4}{\electronvolt}\), \(E_{\mathrm{LUMO}} \approx \SI{-3.0}{\electronvolt}\)) give a HOMO--LUMO gap of approximately \SI{2.4}{\electronvolt}~\cite{Zhang2015}, consistent with the yellow emission under photoexcitation and electroluminescence that underlies its application in organic light-emitting devices~\cite{Friend1999}. In the context of memristive applications, three properties are relevant:
\begin{enumerate}
  \item \textbf{Hole mobility:} adequate hole transport through the bulk of the film provides the baseline electronic-conductance pathway.
  \item \textbf{Electrochemical doping:} susceptibility to $p$-doping by accumulation of cations at the polymer chains (or \textit{n}-doping by anion accumulation) modifies the conductance under the electrochemical-doping mechanism.
  \item \textbf{Solution processability:} solubility in cyclohexanone makes it compatible with the other components of the composite.
\end{enumerate}
...
\Hybrane{} is a hyperbranched polyester-amide synthesised by the polymerisation of a bis-2-oxazoline monomer with a dicarboxylic acid, produced by Polymer Factory (Sweden)~\cite{Froehling2004}. The hyperbranched architecture generates a highly irregular, amorphous macromolecular structure with an exceptionally large number of chain ends. These chain ends, together with the ether and ester oxygens distributed throughout the backbone, provide a dense array of hard Lewis base coordination sites capable of solvating alkali metal cations.

Two properties of \Hybrane{} make it well suited as the ion-conducting component of this composite. First, its glass transition temperature \(T_{\mathrm{g}}\) is sufficiently low that appreciable segmental mobility of the polymer chains is maintained at room temperature, which enables the segmental-motion-assisted ionic transport described in \cref{subsec:ion_conducting_polymers}~\cite{Armand1989}. Second, the hyperbranched architecture suppresses crystallisation: in contrast to linear semicrystalline polymers such as poly(ethylene oxide) (PEO), \Hybrane{} remains amorphous, yielding a homogeneous matrix throughout the film. Crystalline domains in PEO-based solid electrolytes are known to impede ionic transport~\cite{Berthier1983, Lightfoot1993}, and the amorphous host therefore avoids that limitation at the proof-of-concept stage of the thesis.

The ion-transport mechanism in \Hybrane{} is primarily segmental: as polymer chain segments undergo local conformational relaxation, coordinated cations are passed from one oxygen coordination site to the next along the segmental motion pathway. Because this motion is coupled to the segmental dynamics of the polymer host above \(T_{\mathrm{g}}\), the transport follows a non-Arrhenius (super-Arrhenius) temperature dependence of the Williams--Landel--Ferry (WLF) type rather than a simple activated law~\cite{WilliamsLandelFerry1955}. At room temperature, the apparent activation energy for ion hopping in such systems is of order \SI{50}{\kilo\joule\per\mol}~\cite{Gray1991}, consistent with the observation that relatively modest applied voltages (\SIrange{1}{3}{\volt}) are sufficient to displace ions over short distances (sub-\si{\micro\metre}) within the thin-film device.

\subsection{Lithium Triflate: The Ionic Salt}\label{subsec:litf}

\LiTf{} (lithium trifluoromethanesulfonate) is a well-characterised solid electrolyte salt widely employed in polymer batteries and LECs~\cite{Pei1995, Wagberg2008}. Its choice for this system is motivated by three considerations:
\begin{enumerate}
  \item \textbf{Processing compatibility:} it dissolves readily in cyclohexanone and blends homogeneously with \SY{} and \Hybrane{} without phase separation.
  \item \textbf{Hard cation:} the lithium cation is a hard Lewis acid with a small ionic radius (\SIrange{0.59}{0.76}{\angstrom} for four- to six-fold oxygen coordination~\cite{Shannon1976}) and a formal charge of $+1$, giving a charge density well matched to the hard Lewis base oxygen atoms of \Hybrane{}.
  \item \textbf{Soft anion:} the triflate anion has a large, diffuse anionic centre with the negative charge delocalised over the three electronegative fluorine atoms and the sulfonyl oxygen atoms, rendering it a soft Lewis base with low affinity for the Hybrane oxygen coordination sites.
\end{enumerate}
...
The device adopts a vertical sandwich geometry, in which the active layer is placed between a bottom transparent electrode and a top metallic electrode (see \cref{fig:device_schematic}). This 2-T architecture is the simplest memristive configuration available: it contains no gate electrode and requires no transistor to address the device, in contrast to the \textit{three-terminal} (3-T) organic synaptic transistors discussed in \cref{subsec:organic_history}~\cite{Gkoupidenis2015, Xu2016, Gkoupidenis2017}.

\begin{figure}[htbp]
  \centering
  \figplaceholder{7cm}{Two-terminal device architecture and proposed STM/LTM mechanism}
  \caption[Two-terminal device architecture]{Schematic of the two-terminal vertical synaptic device. (a)~The biological synapse used as template. (b)~Device stack: ITO-glass bottom electrode / composite active layer / Ag top electrode (\SI{100}{\nano\metre}). (c)~Molecular structures of the three active-layer components: \SY{} (semiconducting PPV copolymer), \Hybrane{} (hyperbranched polyester-amide ion-transport host), and \LiTf{} (\ce{LiCF3SO3}). (d)~Proposed mechanism: at low voltage the fast triflate anions redistribute (STM); at high voltage the slow \ce{Li+} cations are also displaced (LTM). Adapted from Ref.~\cite{PradoSocorro2022}.}
  \label{fig:device_schematic}
\end{figure}

The 2-T choice matters for two reasons. First, it reduces fabrication complexity, given that the device is made by a sequence of only three depositions (ITO substrate, spin-coated active layer, evaporated metal), which makes the same stack compatible with the comparative corpus of Chapter~3. Second, a 2-T device that shows coexisting \STM{} and \LTM{} without an independent gating signal shows that the memory physics is intrinsic to the active material, rather than being supplied externally by a gate field acting on a separate channel.

The bottom electrode is a pre-patterned layer of \ITO{} deposited on borosilicate glass. ITO combines optical transparency with adequate electronic conductance ($\sigma \approx \SI{e4}{\siemens\per\metre}$), allowing optical inspection of the active layer and characterisation of the device stack by techniques such as UV-Vis spectroscopy and electroluminescence imaging. The work function of ITO ($\Phi_{\mathrm{ITO}} \approx \SI{4.7}{\electronvolt}$) is well matched to the ionisation potential of PPV-family semiconductors~\cite{Ramsdale2002}, facilitating hole injection into the active layer.
...
Following deposition, the coated substrates were transferred to a hotplate within the glovebox and annealed at \SI{75}{\celsius} for \SI{3}{\hour}. The annealing step serves three purposes:
\begin{enumerate}
  \item \textbf{Solvent removal:} it removes residual cyclohexanone solvent, which would otherwise act as a plasticiser and modify the ionic mobility of the matrix.
  \item \textbf{Chain relaxation:} it allows the polymer chains to relax into a morphologically stable configuration.
  \item \textbf{Component mixing:} it promotes intimate mixing of the three components at the molecular scale, maximising the number of \ce{Li+} coordination sites accessible to the triflate anion within the Hybrane matrix.
\end{enumerate}
...
\begin{figure}[htbp]
  \centering
  \figplaceholder{6.5cm}{Current--voltage hysteresis over ten successive sweeps}
...
\begin{figure}[htbp]
  \centering
  \figplaceholder{6cm}{Reversible potentiation/depression under a pulse train}
  \caption[Reversible potentiation and depression]{Analogue multi-state conductance tuning under a pulse-train protocol. Fifty consecutive \SI{+1}{\volt} write pulses continuously potentiate the conductance by more than \SI{200}{\percent}; fifty subsequent \SI{-2}{\volt} pulses depotentiate it back toward the initial state. The smooth, monotonic trajectories demonstrate reversible analogue tuning without abrupt SET/RESET transitions. Data from Ref.~\cite{PradoSocorro2022}.}
...
To quantify the potentiation as a function of pulse number, and to separate the act of writing from the act of reading, a non-perturbing read protocol was adopted; the same protocol is reused for the depotentiation measurement of \cref{subsec:stm_ltm}. The read--write--wait sequence, shown schematically in \cref{fig:stm_ltm}, comprises three steps:
\begin{enumerate}
  \item \textbf{Read.} A reading pulse ($V_{\mathrm{read}} = \SI{0.5}{\volt}$) is applied to establish the initial conductance $G_0$. This reading voltage is intentionally low: the activation threshold for ionic redistribution in this system was determined by independent experiments to lie above \SI{0.7}{\volt}, so $V_{\mathrm{read}} = \SI{0.5}{\volt}$ measures the conductance state without perturbing it.
...
\begin{figure}[htbp]
  \centering
  \figplaceholder{5cm}{Read--write--wait protocol schematic}
  \caption[Read--write--wait protocol]{Pulsed read--write--wait protocol used for the potentiation and retention measurements. A low read pulse ($V_{\mathrm{read}} = \SI{0.5}{\volt}$) establishes the baseline conductance $G_0$; a train of $N_{\mathrm{pulses}}$ write pulses perturbs the ionic configuration; after a controlled waiting time $t_{\mathrm{wait}}$ a second read pulse measures the relaxed state $G_f$. Adapted from Ref.~\cite{PradoSocorro2022}.}
  \label{fig:stm_ltm}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \figplaceholder{6cm}{Potentiation versus number of write pulses}
  \caption[Potentiation versus pulse number]{Dependence of the conductance change $\Delta G = (G_f/G_0)\times 100\%$ on the number of write pulses $N_{\mathrm{pulses}}$ for $V_{\mathrm{write}} = \SI{1}{\volt}$ and $t_{\mathrm{wait}} = \SI{150}{\milli\second}$. The monotonic increase over more than three decades of pulse count demonstrates analogue potentiation. The shaded band is the standard deviation across independent devices. Data from Ref.~\cite{PradoSocorro2022}.}
  \label{fig:npulse}
\end{figure}
...
\begin{enumerate}
  \item \textbf{Initialisation.} The device is initialised to its ground state $S_0$ by applying a reading voltage $V_0 = \SI{+1}{\volt}$ for \SI{1}{\second}.
...
\begin{figure}[htbp]
  \centering
  \figplaceholder{6cm}{EPSC protocol and four distinguishable readout states}
  \caption[Excitatory post-synaptic current]{Excitatory post-synaptic current (EPSC) measurement. Applied voltage and measured current versus time: the ground state $S_0$ is set at \SI{+1}{\volt}; \SI{+2}{\volt} pulses produce excited states $S_1$ and $S_2$; \SI{-2.5}{\volt} pulses produce inhibitory states $S_3$ and $S_4$. The four states are readily distinguishable from the ground state. Data from Ref.~\cite{PradoSocorro2022}.}
  \label{fig:epsc}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \figplaceholder{6cm}{Spike-timing dependent plasticity function}
  \caption[Spike-timing dependent plasticity]{Spike-timing dependent plasticity (STDP). Percentage conductance change $\Delta G$ as a function of the pre/post spike-timing delay $\Delta t$, with each branch fitted to the decaying exponential of \cref{eq:stdp_fit}. Causal timing ($\Delta t < 0$) produces potentiation and anti-causal timing ($\Delta t > 0$) produces depression, with fitted time constant $\tau = \SIrange{85}{90}{\milli\second}$, within the biologically realistic range ($\sim\SI{100}{\milli\second}$). Inset: pre- and postsynaptic voltage waveforms separated by $\Delta t$. Data from Ref.~\cite{PradoSocorro2022}.}
  \label{fig:stdp}
\end{figure}
...
\begin{itemize}
  \item A modest voltage of \SI{1}{\volt} applied across a \SI{209}{\nano\metre} film generates a field of approximately \SI{4.8}{\mega\volt\per\metre}. This is sufficient to overcome the activation barrier for triflate-anion hopping. The fast anionic redistribution produces a rapid conductance change, which then relaxes on the timescale of triflate diffusion through the host. This is the \STM{} regime, with retention \SIrange{10}{15}{\second}, driven primarily by triflate relaxation.

  \item A voltage of \SI{3}{\volt} across the same film generates approximately \SI{14.4}{\mega\volt\per\metre}. This higher field is required to overcome the substantially larger activation barrier for \ce{Li+} hopping out of its \Hybrane{} coordination sites. The cation contribution sets in, produces a delayed and stronger ionic redistribution, and relaxes on the much longer timescale of cation re-coordination. This is the \LTM{} regime, with retention exceeding \SI{45}{\second}, driven primarily by cation redistribution.
\end{itemize}

This account yields a specific, experimentally testable prediction. Replacing \ce{Li+} with a monovalent cation of larger ionic radius (\ce{Na+} or \ce{K+}) should reduce the charge density, and therefore the hardness, of the Lewis acid. The cation--oxygen coordination with the polyether host should weaken accordingly, and the slow fading-memory timescale of the device should shorten as the cation is moved along the \ce{Li+} \(\to\) \ce{Na+} \(\to\) \ce{K+} series. The prediction is tested in Chapter~3 on the main comparative corpus of the thesis: a PEO-based polymer-electrolyte series loaded with lithium, sodium and potassium triflate salts (PEO/LiOTf, PEO/NaOTf, PEO/KOTf) at fixed polymer and salt mass fractions, complemented by a dedicated PEO/LiOTf concentration sub-study. The prediction is addressed there as a statement about the shift of the measured variable-delay depotentiation timescale when the cation is varied at fixed host chemistry, not as a claim about the full synaptic suite reported in the present chapter.
...
\Cref{tab:comparison} places the device reported in this chapter alongside representative organic and hybrid memristive devices from the literature available at the time of publication (2022). The comparison axes are those of \cref{subsec:memristor_params}: device structure (2-T or 3-T), accessible conductance states, demonstrated short- and long-term memory regimes, STDP, and reversibility.
...
Two combinations not previously demonstrated in a single platform stand out from the table. The first is the coexistence of \STM{} and \LTM{} in a single 2-T device that is also fully reversible. Of the prior reports, Liu and co-workers~\cite{Liu2016} comes closest, having shown tunable short- to long-term transitions in a 2-T structure; the redox route, however, modifies the organic active layer irreversibly at each switching cycle, which precludes practical cyclic operation~\cite{Heremans2011}. The present device reaches comparable \STM{}/\LTM{} functionality with full reversibility (the conductance returns to \(G_{0}\) after the reset protocol) and with the retention stability described in \cref{sec:stability}. The second is the demonstration of STDP in a fully reversible 2-T architecture. The earlier organic STDP demonstrations~\cite{Gkoupidenis2015, Kim2018, Seo2019} were all 3-T, with an independent gate voltage providing the timing-dependent rule; here, the same rule emerges from the superposition of pre- and postsynaptic pulses at the single Ag terminal, which is closer in form to the biological synapse where no separate gating terminal is present.

The other axes summarise the operating regime of the device rather than a competitive claim. The operational lifetime (about \SI{300}{\hour} under ambient conditions, without encapsulation) exceeds that of most comparable unencapsulated organic memristive devices by roughly an order of magnitude~\cite{Shih2016}, while remaining far below the \(>10^{10}\) cycles accessible to mature inorganic platforms. The energy per synaptic event (about \SI{50}{\nano\joule}) is consistent with the range reported for other organic ionic devices~\cite{Xiao2016} and substantially below the per-update cost of conventional CMOS synaptic circuits (\(>\SI{1}{\micro\joule}\)), but lies above the picojoule budget of the best inorganic memristors, which reflects the lower switching speed and larger device area of the organic system. Scaling the device area toward the \(\sim\)\SI{100}{\nano\metre\squared} regime considered in the inorganic literature would reduce this gap by several orders of magnitude, although such scaling is not pursued here.

The comparison above retains the device-level metrics conventional in the organic memristive literature of the early 2020s: reversibility, retention window, energy per event, and operational lifetime. Within the thesis-wide argument, those metrics play a narrower role. They establish that the polymer-electrolyte platform introduced here is reversible, dynamically addressable, and stable enough to support the comparative study of Chapter~3 (a PEO-based triflate composition and cation sweep, characterised through the three common dynamical measurements: I--V hysteresis, variable-number-of-pulses potentiation, and variable-delay depotentiation) and the temporal-computing reinterpretation of Chapter~4 built on those measurements. They are not invoked to claim that this device should be judged by the standards of a high-density non-volatile memory technology, nor to suggest that the full synaptic suite reported here is available across the full comparative corpus. Those measurements remain specific to the present SY/Hybrane/LiOTf device.
...
\begin{enumerate}
  \item \textbf{Materials design.} A ternary composite of \SY{} (semiconducting polymer), \Hybrane{} (ion-conducting hyperbranched polyester-amide host), and \LiTf{} (alkali-metal salt) was shown to satisfy the three functional requirements identified in \cref{subsec:design_rationale}: electronic-conductance pathway, ion-transport medium, and ion source. The composition follows the composite logic developed in \cref{subsec:composite_rationale}.

  \item \textbf{Device fabrication.} A 2-T vertical sandwich (ITO/composite/Ag) was fabricated from solution processing and thermal evaporation alone, yielding active layers of \SI{209}{\nano\metre} thickness with root-mean-square surface roughness below \SI{3}{\nano\metre} and batch-to-batch reproducibility within \SI{20}{\percent} on the principal electrical parameters.

  \item \textbf{Current--voltage hysteresis.} The first of the three dynamical measurements carried forward to the comparative corpus: successive triangular voltage sweeps produced a continuously increasing conductance, with a \SI{140}{\percent} increase from cycle 1 to cycle 2 at \SI{1}{\volt} --- the analogue, non-abrupt signature of memristive switching.

  \item \textbf{Variable-number-of-pulses potentiation.} Pulse-train measurements confirmed that the conductance can be potentiated and depotentiated with full reversibility over a range exceeding \SI{200}{\percent}, at an energy cost of order \SI{50}{\nano\joule} per event, and that the conductance change grows monotonically with the number of write pulses, exceeding $10^3$\% for $N_{\mathrm{pulses}} > 10^3$.

  \item \textbf{Variable-delay depotentiation (\STM{} and \LTM{}).} Read-out after a variable delay resolved two distinct memory regimes: an \STM{} regime (\(\tau_{S} = \SIrange{2.5}{3}{\second}\), retention \SIrange{10}{15}{\second}) accessible with \SI{1}{\volt} pulses, and a longer-lived regime denoted \LTM{} (\(\tau_{L} = \SI{4.7}{\second}\), retention \(>\SI{45}{\second}\)) accessible with \SI{3}{\volt} pulses. Both decay curves are well described by a stretched Kohlrausch exponential, in keeping with a heterogeneous distribution of activation barriers within the polymer matrix.

  \item \textbf{EPSC multi-state readout.} The first of the extended protocols specific to this device: four readily distinguishable readout states were demonstrated around the device ground level, with signed EPSC ratios spanning from \(-17\) to \(+16\) relative to \(G_{0}\) under the fixed measurement convention; given the analogue conductance tunability, this is a representative demonstration rather than an intrinsic limit on the number of accessible states.

  \item \textbf{\STDP{}.} An asymmetric anti-Hebbian STDP function was measured, with time constant \(\tau = \SIrange{85}{90}{\milli\second}\), within the biologically realistic range of \(\sim\SI{100}{\milli\second}\)~\cite{Bi1998, Kuzum2013}.

  \item \textbf{Mechanistic account.} The \STM{}/\LTM{} dichotomy was attributed to the differential Lewis acid--base interaction between the mobile ions (\ce{Li+} as hard acid, \ce{CF3SO3-} as soft base) and the host oxygen atoms of \Hybrane{} (hard Lewis bases). The weakly coordinated triflate anion is displaced at low bias and relaxes on a short timescale, giving rise to the \STM{} regime. The strongly coordinated lithium cation requires a higher bias to leave its coordination site and relaxes on a longer timescale, giving rise to the \LTM{} regime. The injection and bulk-transport physics are described jointly by the electrodynamic and electrochemical-doping models, in the reconciling framework of van Reenen and co-workers~\cite{vanReenen2010}.

  \item \textbf{Stability and reproducibility.} Devices remained functional for approximately \SI{300}{\hour} under ambient conditions without encapsulation, with device-to-device variability below \SI{15}{\percent} on the principal electrical parameters.
\end{enumerate}

Taken together, these results establish the SY/Hybrane/LiOTf composite as a richly characterised exemplar of a polymer-electrolyte organic memristive device, and identify the cation--polyether coordination chemistry as the tunable handle on memory timescales. This identification motivates the main comparative study of Chapter~3, which replaces \Hybrane{} with poly(ethylene oxide) and varies both the composition (PEO/LiOTf concentration series) and the cation identity (PEO/LiOTf, PEO/NaOTf, PEO/KOTf at fixed polymer and salt mass fractions). In Chapter~3, the hypothesis is tested through the three common dynamical measurements (I--V hysteresis, variable-number-of-pulses potentiation, and variable-delay depotentiation) that are available for every device in the comparative corpus. The richer synaptic metrics reported in the present chapter (EPSC, separated \STM{}/\LTM{} retention, and STDP) are not propagated to the later chapters and serve there only as lithium-device priors and sanity checks. Beyond this comparative handoff, the volatile two-timescale behaviour established here is the experimental anchor for the temporal-computing reinterpretation of Chapter~4, in which behavioural models fitted to the Chapter~3 corpus drive simulations of a heterogeneous physical reservoir, a coincidence detector, and a multi-timescale transient filter bank. In those paradigms, the fading memory of the device is the active computational resource rather than a retention defect to be suppressed.
