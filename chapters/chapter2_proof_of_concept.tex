...
The chapter shows that the composite exhibits, in a single 2-T platform: current--voltage hysteresis characteristic of memristive switching; analogue multi-state conductance tunable by pulsed voltage protocols; excitatory post-synaptic current (\EPSC{}) responses, demonstrated here with four readily distinguishable readout states; a short-lived and a longer-lived memory regime, denoted \STM{} and \LTM{} following the convention of the artificial-synapse literature; and spike-timing dependent plasticity (\STDP{}) with a time constant in the biologically realistic range. The interpretation tying these observations together is developed in \cref{sec:mechanism}: the fast component is associated with displacement of the weakly bound triflate anion, while the slow component is associated with displacement of the lithium cation from its hard oxygen coordination environment. The same interpretation yields the cation-substitution prediction tested in Chapter~3.
