...
The active-layer logic adopted here is the composite logic of \cref{subsec:composite_rationale}. The three functional roles --- electronic transport, ionic transport, and mobile-charge supply --- are assigned to three different components, so that each role can be tuned chemically without redesigning the rest of the stack. \SY{} supplies the electronic-conductance pathway. \Hybrane{} supplies an amorphous, low-crystallinity ion-transport host with a high density of hard oxygen donors. \LiTf{} supplies a hard alkali cation paired with a soft, charge-delocalised anion, so that the cation--host and anion--host interactions are deliberately asymmetric in the hard--soft acid--base sense of Pearson~\cite{Pearson1963}. This asymmetry is what links the chemistry of the active layer to the two memory regimes reported below.

The chapter shows that the composite exhibits, in a single 2-T platform: current--voltage hysteresis characteristic of memristive switching; analogue multi-state conductance tunable by pulsed voltage protocols; excitatory postsynaptic current (\EPSC{}) responses with four readily distinguishable readout states; a short-lived and a longer-lived memory regime, denoted \STM{} and \LTM{} following the convention of the artificial-synapse literature; and \STDP{} with a time constant in the biologically realistic range. The interpretation tying these observations together is developed in \cref{sec:mechanism}: the fast component is associated with displacement of the weakly bound triflate anion, while the slow component is associated with displacement of the lithium cation from its hard oxygen coordination environment. The same interpretation yields the cation-substitution prediction tested in Chapter~3.

Within the arc of the thesis, the present chapter is deliberately narrow in role. It is the only chapter in which the full synaptic suite --- EPSC, separated short- and long-term retention, STDP, and impedance spectroscopy --- is treated as primary evidence. Chapters~3 and~4 rely instead on three common dynamical measurements (I--V hysteresis, variable-number-of-pulses potentiation, and variable-delay depotentiation) that are available across the full comparative corpus, and the richer metrics reported here enter the later chapters only as lithium-device priors and sanity checks. The two specific handoffs onto the rest of the thesis are taken up at the end of the chapter.
...
\begin{enumerate}
  ...
  \item \textbf{Ion-transport medium:} a polymer electrolyte provides a structural environment in which dissolved ions remain mobile under an applied field, yet are sufficiently retarded by intermolecular interactions to leave behind persistent conductance states once the field is removed.
  \item \textbf{Ion source:} a dissociated salt provides the mobile charge carriers (a cation and an anion) whose differential response to the field is responsible for the distinct memory regimes reported in \cref{sec:elec_char}.
\end{enumerate}

The composition adopted here is built around a deliberate asymmetry in Lewis acid--base character between the two ions of the salt. Lithium triflate (\LiTf{}) is chosen because \ce{Li+} is a hard Lewis acid with a high charge-to-radius ratio (\(\lambda = z/r \approx \SI{1.36}{\per\angstrom}\), with \(r_{\mathrm{Li^+}} = \SI{0.73}{\angstrom}\)), whereas the triflate anion \ce{CF3SO3-} is a soft Lewis base with a diffuse, low-charge-density anionic centre. The ion-transport polymer, \Hybrane{}, contains a high density of ether and ester oxygen atoms that act as hard Lewis bases. Under the hard--soft acid--base (HSAB) reasoning of Pearson~\cite{Pearson1963} already used in \cref{subsec:ion_conducting_polymers}, hard acids associate preferentially with hard bases. The \ce{Li+} cations are therefore tightly coordinated by the oxygen atoms of \Hybrane{}, while the \ce{CF3SO3-} anions interact only weakly with the host. Under bias, the weakly coordinated triflate anions are displaced at low voltage, while the strongly coordinated lithium cations require a substantially higher field to break free of their coordination environment. This differential mobility is the molecular origin of the two memory regimes (\STM{} and \LTM{}) reported below.
...
This chapter has reported the design, fabrication, and characterisation of a fully reversible 2-T organic memristive device that supports the full synaptic suite (\STM{}, \LTM{}, and \STDP{}) within a single polymer-composite active layer. The main contributions are as follows.
...
\begin{enumerate}
  ...
  \item \textbf{Analogue multi-state switching.} Current--voltage hysteresis measurements showed a continuously increasing conductance under successive voltage sweeps, with a \SI{140}{\percent} increase from cycle 1 to cycle 2 at \SI{1}{\volt}. Pulse-train measurements confirmed that the conductance can be potentiated and depotentiated over a range exceeding \SI{200}{\percent}, at an energy cost of order \SI{50}{\nano\joule} per event.

  \item \textbf{EPSC and multi-state memory.} Four readily distinguishable readout states were established around the device ground level, with signed EPSC ratios spanning from \(-17\) to \(+16\) relative to \(G_{0}\) under the fixed measurement convention.

  \item \textbf{\STM{} and \LTM{}.} Two distinct memory regimes were characterised: an \STM{} regime (\(\tau_{S} = \SIrange{2.5}{3}{\second}\), retention \SIrange{10}{15}{\second}) accessible with \SI{1}{\volt} pulses, and a longer-lived regime denoted \LTM{} (\(\tau_{L} = \SI{4.7}{\second}\), retention \(>\SI{45}{\second}\)) accessible with \SI{3}{\volt} pulses. Both decay curves are well described by a stretched Kohlrausch exponential, in keeping with a heterogeneous distribution of activation barriers within the polymer matrix.
