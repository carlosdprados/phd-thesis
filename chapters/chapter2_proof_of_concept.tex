...
For longer waiting times, the expected exponential relaxation of conductance is observed as the ionic configuration relaxes back toward equilibrium by thermal diffusion. The Supporting Information of Ref.~\cite{PradoSocorro2022} reports least-squares fits in IGOR Pro 8 to a standard exponential:
...
\begin{enumerate}
  ...
  \item \textbf{Variable-delay depotentiation (\STM{} and \LTM{}).} Read-out after a variable delay resolved two distinct memory regimes: an \STM{} regime (\(\tau_{S} = \SIrange{2.57}{3.01}{\second}\), retention \SIrange{10}{15}{\second}) accessible with \SI{1}{\volt} pulses, and a longer-lived regime denoted \LTM{} (\(\tau_{L} = \SI{4.73}{\second}\), retention \(>\SI{45}{\second}\)) accessible with \SI{3}{\volt} pulses. These values come from standard-exponential fits with $\beta=1$ fixed; a free stretch exponent and fit-quality statistic were not reported for the published proof-of-concept traces.
  ...
  \item \textbf{Stability and reproducibility.} Devices retained their performance over an ambient shelf lifetime of approximately \SI{300}{\hour} without encapsulation (cycling endurance, a distinct metric, was not characterised). The archived pulse-train population shows about \SI{10}{\percent} low-pulse-count variability across five independent pixels on NM\_v055 (median RSD \SI{8.9}{\percent}, maximum \SI{11.8}{\percent} for $N_{\mathrm{pulses}} < 100$); broader batch reproducibility is addressed by the campaign analysis in \cref{ch:bridge}.
\end{enumerate}
