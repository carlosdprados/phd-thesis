...
\paragraph{A single dataset, and a reported null.} The affective results rest on one corpus (\WESAD{}). The central negative result --- that timescale heterogeneity does not separably improve final-label classification --- is reported with error bars across seeds and sampling intervals rather than omitted, because it is informative: it localises exactly where heterogeneity helps and where it does not.
