#!/usr/bin/env python3
"""EIS corroboration of the composition -> ionic-transport mechanism (Chapter 3).

Run from the repo root:  python3 scripts/ch3_eis.py
Reads the (un-versioned) experimental DATABASE in ../Nanomem_Devices_Library/DATABASE
and the per-cell fading-memory ladder in handouts/ch3_decay_by_cell.csv
(written by scripts/ch3_4_dynamics_fits.py -- run that first).
...
thalf = {}
with open(os.path.join(OUT, "ch3_decay_by_cell.csv"), newline="") as fh:
...
def main():
    ...
    with open(os.path.join(OUT, "ch3_eis_by_cell.csv"), "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=list(rows[0].keys()))
        w.writeheader(); w.writerows(rows)
    print(f"\nWrote {OUT}/ch3_eis_by_cell.csv")
