#!/usr/bin/env python3
"""Chapter 4 dynamics fits: stretched-exponential fading-memory decay (DELAYTIME)
and pulse-potentiation descriptors (PULSES), per composition/cation cell.

Run from the repo root:  python3 scripts/ch4_dynamics_fits.py
...
- PNG-derived human curation (verdict + hand-picked points per device/measurement)
  is read from handouts/ch4_png_qa_curation.csv and applied: 'discard' drops the
...
def main():
    ...
    with open(os.path.join(OUT, "ch4_decay_by_cell.csv"), "w", newline="") as fh:
    ...
    with open(os.path.join(OUT, "ch4_pulses_by_cell.csv"), "w", newline="") as fh:
