#!/usr/bin/env python3
"""Chapter 4 --- electrode (Ag vs Au) contrast at the lead composition cell.

The quantitative spine of Chapter 4 (the SY/PEO/LiOTf composition grid) and the
...
rest of Chapter 4 uses:
  ...
  (3) Independent-electrode replication of the chemistry claims. The host effect
      ...
      potentiation --- strengthening the Chapter 4 chemistry landscape on a
      second electrode.

Run from the repo root:  python3 scripts/ch4_electrode.py
Writes handouts/ch4_electrode_by_cell.csv and prints the tables used in the text.
"""
import csv
import collections
import os
...
def main():
    ...
    with open(os.path.join(OUT, "ch4_electrode_by_cell.csv"), "w", newline="") as fh:
    ...
    print("\nReference (Ag, from ch4_decay_by_cell.csv): PEO/Li 0.3/0.09 t_half_med ~ 19.2 s (n=3).")
