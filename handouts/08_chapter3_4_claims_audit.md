...
| C7 | Reservoir simulations grounded in measured cells | 4 | ✅/⏳ | MC/NARMA + WESAD scripts and figures now exist (`scripts/ch4_*.py`, `figures/chapter4/`). Active framing is handout 12: heterogeneity win is robust on MC benchmarks and now also supported on WESAD physiological temporal-context reconstruction; WESAD final-label classification shows real affective computing and a modest memory benefit, but no separable heterogeneity win. Coincidence is cut; filter-bank logic is folded into reservoir framing. |
